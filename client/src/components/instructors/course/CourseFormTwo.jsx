...
import { updateCourseImage,updatePromotionalVideo } from '../../../redux/createCourse'
import { imageValidation,videoValidation} from '../../../validations/FormValidations';

function CourseFormTwo({formik}) { 
  const {course_image,promo_video} = useSelector((state)=>state.createCourse);
  ...
  function handleImageUpload(e){
    console.log('hello')
      ...
      if(!imageValid.valid){
        ...
        dispatch(updateCourseImage(e.target.files[0]))
        setError(null)
      }
  }
  function handleVideoUpload(e){
      ...
      if(!videoValid.valid){
        ...
        dispatch(updatePromotionalVideo(e.target.files[0]))
        setError(null)
      }
  }
  return ( 
    <div className="font-poppins w-full h-full flex  place-content-start place-items-center ">
        <div  className='w-3/5 flex flex-col place-items-start place-content-center px-3'>
            ...
            <img src={!course_image ? "/instructor/image-placeholder.png" : URL.createObjectURL(course_image)} alt="image_preview" className="border-2 border-gray-100 p-5 my-5"/>
            <input
